...
export class DrizzleStorage implements IStorage {
  ...
  async createVendor(vendor: InsertVendor, password?: string): Promise<Vendor> {
    // 1. Create vendor record in central DB
    ...
    // 2. Create Neon Auth user for the vendor
    let authUid: string | null = null;
    if (password) {
      const authEmail = `${vendor.phone}@gmail.com`;
      const authPassword = password || vendor.phone;
      const result = await createNeonAuthUser(authEmail, authPassword, `${vendor.firstName} ${vendor.lastName}`.trim());
      if (result.id) {
        authUid = result.id;
        await db.update(vendors).set({ authUid }).where(eq(vendors.id, newVendor.id));
      } else if (result.error) {
        console.error("Error creating vendor auth user:", result.error);
      }
    }
  ...
  async updateVendor(id: number, vendor: Partial<InsertVendor>, options?: { password?: string }): Promise<Vendor> {
    ...
    if (current?.authUid) {
      ...
      if (Object.keys(authUpdates).length > 0) {
        ...
        if (authResult.error) {
          console.error("Error syncing vendor auth user:", authResult.error);
        }
      }
    }
